...
//   3. If detected → calls /api/lookup-barcode → enters REVIEW step
//   4. Review step: user can edit the product name before applying
//   5. "Use This" → calls onResult with (possibly corrected) data
//   6. "Try Again" → returns to scan state
//
...
export default function BarcodeScanner({ onResult, onClose }: Props) {
  ...
  // After a successful lookup, we hold the result here for the user to review/edit
  ...
  // Reset the review step back to the initial scan state.
  const resetToScan = () => {
  ...
  // On success, enters the review step instead of applying immediately.
      ...
      <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">
              {pendingResult ? 'Review Product' : 'Scan Barcode'}
            </h2>
        ...
        {pendingResult ? (
          <div className="space-y-4">
            {/* Editable product name — often the most important thing to fix */}
            <div>
